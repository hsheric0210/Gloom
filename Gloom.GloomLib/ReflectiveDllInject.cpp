...
ULONGLONG ReflectiveDllInject(DWORD pid, LPCSTR reflectiveLoaderProcName, size_t dllDataSize, LPVOID dllDataHeap, size_t dllEntryParameterSize, LPVOID dllEntryParameterHeap)
{
	__try
	{
		...
		std::uniform_int_distribution<BYTE> fillData(0, UCHAR_MAX);
		auto pageShift = 4096 * shiftAmount(mt);
		LPVOID remoteDLLMemory = myVirtualAllocEx(processHandle, NULL, dllDataSize + pageShift, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		if (!remoteDLLMemory)
		...
		BOOL state = myWriteProcessMemory(processHandle, remoteParameterMemory, dllEntryParameterHeap, dllEntryParameterSize, nullptr);
		if (!state)
